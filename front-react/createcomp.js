...
if (!componentName) {
  console.error("❌ Debes indicar el nombre del componente. Ej: node createComponent Login");
  process.exit(1);
}
if (!/^[A-Z]/.test(componentName)) { // Primera letra mayúscula
  console.error("❌ El nombre del componente debe comenzar con mayúscula. Ej: Login");
  process.exit(1);

}
// Rutas preestablecidas
const baseDirJsx = path.join(__dirname, "src", "Components", componentName);
const baseDirCss = path.join(__dirname, "src", "Components/CSS", componentName);
